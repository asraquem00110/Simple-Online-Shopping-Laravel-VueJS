<template>
	<div class="">
			...
			<span class="pageheader"><i class="fa fa-plus"></i> Create Product</span><hr/>
		<div class="panel" id="userpanel">
			...
			<div class="panel-body">
					...
					<label>Price</label>
					<textarea class="form-control" v-model="product.price"></textarea>
					<span ref="priceerror" :style="errorstyle"></span><br/>

					<label>Category</label>
					<select class="form-control" v-model="product.category">
						<option v-for="(category,index) in product_CategoryList" :key="index" :value="category.id">{{category.category}}</option>
					</select>
					<span ref="categoryerror" :style="errorstyle"></span><br/>

					<label>Brand</label>
					<select class="form-control" v-model="product.brand">
						<option v-for="(brand,index) in product_BrandList" :key="index" :value="brand.id">{{brand.brand}}</option>
					</select>
					<span ref="branderror" :style="errorstyle"></span><br/>
				</div>
			</div>

				<button @click="saveProduct" class="btn btn-primary pull-right"><span class="fa fa-check"></span> Save</button>
	...
	export default {
		...
		methods: {
            ...
            saveProduct: function(e){
            	...
            	fd.append("price",app.product.price)
            	...
            	axios.post(`/api/saveProduct`,fd,config)
            		.then((res)=>{	
            			...
            			if(res.data.errors){
            			  ...
            			  app.product.brand = ''
            			  app.product.imgfile = ''
            			  app.product.img =  '../../../upload/plus.png'
		...
		computed: {
			    ...
			    // ...mapState({
			    // 	product_CategoryList: state => state.product.product_categories,
			    // })

			    // ...mapState('product',{
			    // 	product_CategoryList: state => state.product_categories,
			    // })

			   // product_CategoryList: function(){
			   // 	return this.$store.getters['product/product_CategoryList']
			   // },

			   // product_BrandList: function(){
			   // 	return this.$store.getters['product/product_BrandList']
			   // }
