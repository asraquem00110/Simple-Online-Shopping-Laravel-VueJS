<template>
	...
	<div class="">
				<span class="pageheader"><i class="fa fa-user"></i> Customer(s) List</span><hr/>
			...
			<input type="text" v-model="search" placeholder="Search via name..." class="form-control" style="width: 50% !important">
	...
	export default {
			...
			methods: {
				getUserlist: function(){
					...
					axios.get(`/api/getCustomerList`,{headers: {'Accept': 'application/json','Authorization': 'Bearer '+ app.$store.state.token}})
					.then((res)=>{
				...
				sortbySearch: function(){
							...
							axios.get(`/api/getCustomerBySearch/${this.search}`,{headers: {'Accept': 'application/json','Authorization': 'Bearer '+ this.$store.state.token}})
							.then((res)=>{
								this.users = res.data.users
							})
							.catch((err)=>{
